...
class KnowledgeBaseDB {
  ...
  async searchSimilarDocuments(queryEmbedding, threshold = 0.1, limit = 5) {
    try {
      ...
      const { data: embeddingCheck, error: checkError } = await this.supabase
        .from('documents')
      ...
      if (!checkError && embeddingCheck) {
        ...
        embeddingCheck.forEach(doc => {
          console.log(`  - Title: "${doc.title}", Has embedding: ${!!doc.embedding}, Embedding length: ${doc.embedding?.length || 0}`);
        });
      }
